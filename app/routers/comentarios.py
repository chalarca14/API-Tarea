...
@router.get("", response_model=List[schemas.ComentarioResponse], summary="Listar los comentarios")
def get_comentarios(
...
@router.get("/{id}", response_model=schemas.ComentarioResponse, summary="Obtener un comentario por ID")
def get_comentario(id: int, db: Session = Depends(get_db)):
...
@router.post("", response_model=schemas.ComentarioResponse, status_code=status.HTTP_201_CREATED, summary="Crear un comentario")
def create_comentario(comentario_in: schemas.ComentarioCreate, db: Session = Depends(get_db)):
...
@router.put("/{id}", response_model=schemas.ComentarioResponse, summary="Actualizar un comentario")
def update_comentario(id: int, comentario_in: schemas.ComentarioUpdate, db: Session = Depends(get_db)):
    comentario = db.query(models.Comentario).filter(models.Comentario.id == id).first()
    if not comentario:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Comentario no encontrado")
...
@router.delete("/{id}", status_code=status.HTTP_200_OK, summary="Eliminar un comentario")
def delete_comentario(id: int, db: Session = Depends(get_db)):
    comentario = db.query(models.Comentario).filter(models.Comentario.id == id).first()
    if not comentario:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Comentario no encontrado")
